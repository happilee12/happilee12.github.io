...
### 2. 여러 변수가 값 할당
array destructuring을 활용하면 한 줄 만에 여러 변수에 값을 할당할 수 있다.
...
### 9. 여러 줄의 문자열
여러줄에 걸친 문자열에 대해서 일반적으로는 `+` 연산자와 `\n`문자를 사용하는데, 백틱(`) 을 사용하면 더 쉬워진다.
...
### 10. 여러 조건 확인
여러개 중 하나의 값과 매칭하는지 여부를 볼 때에는, 매칭하는 값들을 array에 넣고 `indexOf()`나 `includes()`를 사용할 수 있다.
```javascript
//Longhand
...
하지만 `+`연산자를 문자열 앞에 붙여주는 방식으로도 문자열을 숫자로 변환할 수 있다.
```javascript
//Longhand
...
### 13. 문자열 여러번 반복
문자열을 여러번 반복하기 위해 `for`룹을 사용할 수도 있지만, `repeat()` 메소드를 사용하면 한줄에 처리할 수 있다.
...
거듭제곱 연산에 `Math.pow()`를 사용할 수 있지만, `**`를 활용하면 더 짧게 쓸 수 있다.
```javascript
//Longhand
...
const floor = ~~6.8; // 6
```
**Caleb**님이 남겨준 comment: double NOT 비트연산자는 32 비트 정수에만 적용된다. (ex. (2**31)-1 = 2147483647 ) 따라서,2147483647보다 큰 수에는 비트 연산자 대신 Math.floor() 를 사용해야 한다


### 16. 배열에서 최대, 최솟값 찾기
loop을 통해 배열 내 값을 하나하나 확인하며 최대/최솟값을 찾는 방법도 있고, `Array.reduce()`를 활용하는 방법도 있다.
하지만 `...(spread operator)`를 사용하면 한 줄에 처리할 수 있다.
...
### 17. For loop
전통적인 `for`룹 대신 `for ... of` 룹이나, `for ... in` 룹을 사용하여 배열을 순회할 수도 있다.
```javascript
let arr = [10, 20, 30, 40];
//Longhand
